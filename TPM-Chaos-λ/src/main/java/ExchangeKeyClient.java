...
public class ExchangeKeyClient extends SensorUtils {
    ...
    public String generateKey_Client() throws Exception {
        ...
        while (true) {
            short out1 = tpm.getOutput(input);
            //socketConnection = new SocketConnection(port,host);
            msg1 = socketConnection.send(out1+","+short2string(tpm.getSecretKey()).hashCode()+'\0',msg -> {
                //System.out.println("接收到的数据："+msg);
            });
        ...
        System.out.println("本次同步时间为："+(endTime-startTime)+"ms");
        key = KeyGeneration.generateKey(tpm.getSecretKey(),bound);
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("输入K,L,N");
        ExchangeKeyClient.k = scanner.nextInt();
        ExchangeKeyClient.bound = scanner.nextInt();
        ExchangeKeyClient.n = scanner.nextInt();

        exchangeKey.createSocket(8089,ipaddress);
        String key_client = exchangeKey.generateKey_Client();
        exchangeKey.command.addkey(exchangeKey.host,key_client);

        exchangeKey.command.loop_client(exchangeKey.host,8089);
